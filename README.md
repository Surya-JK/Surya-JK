<div align="center">

# Hey there, I'm Surya JK 👋

**AI & Data Science undergrad · ML Engineer · Full-stack Developer · Data Analyst**

*"I build things that think — from computer vision models to full-stack apps and data dashboards."*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-suryajk-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/suryajk)
[![Portfolio](https://img.shields.io/badge/Portfolio-surya--jk-orange?style=flat-square&logo=firefox&logoColor=white)](https://surya-jk-portfolio.onrender.com)
[![Email](https://img.shields.io/badge/Email-jksurya06@gmail.com-D44638?style=flat-square&logo=gmail&logoColor=white)](mailto:jksurya06@gmail.com)
[![CGPA](https://img.shields.io/badge/CGPA-9.08-brightgreen?style=flat-square)](https://github.com/Surya-JK)

</div>

---

## 🧠 About me

I'm a B.Tech student in **Artificial Intelligence & Data Science** at Saveetha School of Engineering, Chennai.  
I love turning raw data and complex problems into real-world solutions — whether that's training a CV model, wiring up a full-stack app, or building a dashboard that makes data actually readable.

- 🔭 Currently building: AI/ML projects and sharpening full-stack skills
- 🌱 Always learning: latest in ML, cloud, and system design
- 🤝 Open to: internships, collaborations, and interesting problems
- 🏏 Outside code: cricket and good music keep me sane

---

## 🛠 Tech Stack

```
Languages     →  Python  ·  Java  ·  JavaScript
Frontend      →  React.js  ·  HTML  ·  CSS  ·  Bootstrap
Databases     →  MySQL  ·  SQLite  ·  Oracle SQL
ML / AI       →  Computer Vision  ·  Data Analysis
Tools         →  Docker  ·  Apache Superset  ·  Git
Cloud         →  Oracle Cloud  ·  Forage (Verizon)
```

---

## 🚀 Featured Projects

### 🚗 [Car Damage Assessment](https://github.com/Surya-JK/Kaspon-Techworks-Intern-Project-Car-Damage-Assesment)
> Built during internship at Kaspon Techworks · Nov–Dec 2025

AI model using **Computer Vision** to detect and assess vehicle damage from images. End-to-end ML pipeline from data to prediction.

`Python` `Computer Vision` `Machine Learning`

---

### 📊 [FP Tickets Dashboard](https://github.com/Surya-JK/Kaspon-Techworks-Intern-Project-FP-Tickets-Dashboard-using-Apache-Superset)
> Built during internship at Kaspon Techworks · Nov–Dec 2025

Interactive data visualization dashboard for field performance tickets using **Apache Superset**.

`Apache Superset` `SQL` `Data Visualization`

---

### 🌐 [BonTon Chatbot & E-commerce App](https://github.com/Surya-JK/Intern-Project-Prototype-BonTon-Website)
> Full-stack internship at BonTon Softwares · Jul–Oct 2025

Full-stack web projects including a chatbot, e-commerce app, and company prototype site — covering frontend and backend.

`React v19` `MySQL` `JavaScript` `Bootstrap`

---

### 🤖 [Virtual Assistant](https://github.com/Surya-JK/Virtual-Assistant) · 🌦 [Weather Monitoring System](https://github.com/Surya-JK/Weather-Monitoring-System)
Personal Python projects — a voice-enabled virtual assistant and a real-time weather tracker.

`Python`

---

## 🏅 Certifications

| Certification | Issuer |
|---|---|
| Oracle SQL Specialist · **Score: 86%** | Oracle |
| Data Science Foundations — Level 1 | IBM |
| Gemini Certified Student | Google |
| Python & Java | HackerRank |
| Ethical Hacking | NPTEL |
| Cyber Security and its Privacy | NPTEL |
| Cloud Platform Job Simulation | Verizon · Forage |

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Surya-JK&show_icons=true&theme=github_dark&hide_border=true" height="150"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Surya-JK&layout=compact&theme=default&hide_border=true" height="150"/>
</div>

---

<div align="center">
  <i>Always open to meaningful conversations, collabs, and a good cricket debate.</i><br><br>
  <b>Let's build something together →</b> <a href="mailto:jksurya06@gmail.com">jksurya06@gmail.com</a>
</div>
